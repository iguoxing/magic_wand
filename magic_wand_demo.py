...
class Segment_my(object):
    ...
    def update(self, param, flag=False):
        ...
        if self.param != param or flag:
            ...
            # HLS 变换
            hue, saturation, lightness = hue / 100, saturation / 100, lightness / 100
            hls_image = cv2.cvtColor(self.image,cv2.COLOR_BGR2HLS)
            hls_image = hls_image.astype(np.float32)
            hls_image = hls_image*np.array([hue,lightness,saturation])
            hls_image = np.clip(hls_image,0,255).astype(np.uint8)
            self.image = cv2.cvtColor(hls_image,cv2.COLOR_HLS2BGR)
            '''
...
# cv2.createTrackbar('Hue', 'image', 100, 150, nothing)
# cv2.createTrackbar('Saturation', 'image', 100, 150, nothing)
# cv2.createTrackbar('Lightness', 'image', 100, 150, nothing)
...
while(1):
    ...
    # lightness = cv2.getTrackbarPos('Lightness', 'image')
